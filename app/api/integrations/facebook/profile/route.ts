...
import {
  ...
  updateFacebookPageProfile,
} from "@/lib/integrations/facebook-page-profile-server";
...
export async function GET(req: Request) {
  ...
  const result = await fetchFacebookPageProfile(auth.ctx.restaurantId);
  if (!result.ok) {
...
export async function PATCH(req: Request) {
  ...
  const auth = await authorizeFacebookRestaurantRoute(body.restaurantId ?? null);
  if (!auth.ok) {
    return Response.json({ error: auth.error }, { status: auth.status });
  }
  ...
  const result = await updateFacebookPageProfile(auth.ctx.restaurantId, profile);
  if (!result.ok) {
    return Response.json({ ok: false, error: result.error }, { status: 422 });
  }
