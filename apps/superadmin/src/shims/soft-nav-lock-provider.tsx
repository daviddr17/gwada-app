...
import { prefetchSuperadminSpaHref } from "../navigation/prefetch-superadmin-route";
...
export function SoftNavLockProvider({ children }: { children: ReactNode }) {
  ...
  const pathname = tanstackLocationToZonePath("/superadmin", location.pathname);
  ...
  const tryAcquireNavLock = useCallback(
    (_event: { preventDefault: () => void }, targetHref: string) => {
      ...
      prefetchSuperadminSpaHref(targetHref);
      pendingTargetRef.current = target;
