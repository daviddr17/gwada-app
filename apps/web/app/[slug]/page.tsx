...
import { buildPublicRestaurantJsonLd } from "@/lib/restaurant/public-restaurant-json-ld";
import {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const origin = getPublicSiteUrl()?.replace(/\/+$/, "") ?? "";
  const canonical = origin ? `${origin}/${profile.slug}` : undefined;
...
export default async function PublicRestaurantProfilePage({ params }: PageProps) {
  ...
  const jsonLd = buildPublicRestaurantJsonLd(profile);

  // LCP: responsive Cover/Avatar — Gwada-Icon nicht mit high priority konkurrieren lassen
  if (profile.coverUrl) {
    preload(profile.coverUrl, {
      as: "image",
      fetchPriority: "high",
      ...(profile.coverSrcSet
        ? {
            imageSrcSet: profile.coverSrcSet,
            imageSizes: PUBLIC_PROFILE_COVER_SIZES,
          }
        : {}),
    });
  } else if (profile.avatarUrl) {
    preload(profile.avatarUrl, {
      as: "image",
      fetchPriority: "high",
      ...(profile.avatarSrcSet
        ? {
            imageSrcSet: profile.avatarSrcSet,
            imageSizes: PUBLIC_PROFILE_AVATAR_SIZES,
          }
        : {}),
    });
  }
