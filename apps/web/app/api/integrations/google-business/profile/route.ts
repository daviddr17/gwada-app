...
import {
  fetchGoogleBusinessLocationProfile,
  updateGoogleBusinessLocationProfile,
} from "@/lib/integrations/google-business-profile-server";
...
export async function GET(req: Request) {
  ...
  const result = await fetchGoogleBusinessLocationProfile(auth.ctx.restaurantId);
  if (!result.ok) {
...
export async function PATCH(req: Request) {
  ...
  const result = await updateGoogleBusinessLocationProfile(
