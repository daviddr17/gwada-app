...
import {
  fetchPlatformComplianceChecklistTemplates,
...
export async function GET(req: Request) {
  ...
  const { templates, error } = await fetchPlatformComplianceChecklistTemplates(admin, {
...
export async function POST(req: Request) {
  ...
  const { template, error } = await savePlatformComplianceChecklistTemplate(admin, input);
  if (error || !template) {
