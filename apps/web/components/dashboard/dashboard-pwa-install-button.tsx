...
function isStandaloneDashboardPwa(): boolean {
...
export function DashboardPwaInstallButton({ className }: { className?: string }) {
  ...
  useEffect(() => {
    setInstalled(isStandaloneDashboardPwa());
  }, []);
  ...
  return (
    <>
      <Button
        ...
        variant="outline"
        size="icon-sm"
        className={cn("shrink-0 rounded-full border-border/60", className)}
        aria-label="Dashboard zum Home-Bildschirm hinzufügen"
      ...
      <Drawer open={iosHintOpen} onOpenChange={setIosHintOpen} direction="bottom">
        <DrawerContent className={drawerContentClassName("filter")}>
          <DrawerHeader>
            ...
            <DrawerDescription>
              So startest du das Dashboard wie eine App auf dem iPad oder iPhone.
            </DrawerDescription>
          </DrawerHeader>
