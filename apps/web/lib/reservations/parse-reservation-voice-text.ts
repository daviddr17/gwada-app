...
/** Kurzes „Ja“ / „Anlegen“ zur Bestätigung im Nachfrage-Sheet. */
export function isReservationVoiceConfirmUtterance(text: string): boolean {
  const t = text
    .trim()
    .toLowerCase()
    .replace(/[.!?,;:]+$/g, "")
  ...
  if (/^(bitte\s+)?anlegen\b/.test(t)) return true;
