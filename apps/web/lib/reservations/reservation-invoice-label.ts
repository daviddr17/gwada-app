import type { AccountingInvoiceRow } from "@/lib/types/accounting";

export function formatReservationInvoiceOptionLabel(
  q: Pick<
  ...
  const num = q.voucher_number?.trim() || "Rechnung";
...
export type ReservationInvoiceJoin = {
  id: string;
...
export function mapReservationInvoiceJoin(
  raw: unknown,
): ReservationInvoiceJoin | null {
...
export function formatReservationInvoiceJoinLabel(
  q: ReservationInvoiceJoin | null | undefined,
): string {
  if (!q) return "";
  const num = q.voucher_number?.trim() || "Rechnung";
