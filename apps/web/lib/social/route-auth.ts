...
export async function authorizeSocialNewsRestaurant(
  restaurantId: string,
...
export async function authorizeSocialBrandKitRestaurant(
  restaurantId: string,
): Promise<
  | {
      ok: true;
...
> {
  ...
  const branding = await authorizeRestaurantModule(
    restaurantId,
    "settings.branding",
  );
  if (branding.ok) return branding;

  return authorizeModuleCrud(restaurantId, "news", "create");
}
