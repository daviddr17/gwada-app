...
export function SettingsIntegrationSaveProvider({
  children,
  onAfterSave,
}: {
  children: ReactNode;
...
export function useSettingsIntegrationSave() {
  const ctx = useContext(RegistryContext);
  if (!ctx) {
    throw new Error(
      "useSettingsIntegrationSave requires SettingsIntegrationSaveProvider",
    );
  }
  return ctx;
}

export function useRegisterSettingsIntegrationSave(
  id: string,
  isDirty: boolean,
  save: SaveHandler,
) {
  const { register } = useSettingsIntegrationSave();
