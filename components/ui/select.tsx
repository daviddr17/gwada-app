    ...
    <SelectPrimitive.Portal container={portalContainer}>
      ...
      >
        ...
        >
          ...
          <SelectPrimitive.List className="min-h-0 flex-1 scroll-py-2 overflow-y-auto overscroll-contain px-1.5 py-2 outline-none">
            {children}
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex min-h-11 w-full cursor-default items-center gap-2 rounded-xl py-2.5 pr-10 pl-3 text-[15px] leading-snug text-popover-foreground outline-hidden select-none hover:bg-muted/70 hover:text-foreground focus:bg-muted/80 focus:text-foreground data-disabled:pointer-events-none data-disabled:opacity-45 data-highlighted:bg-muted/80 data-highlighted:text-foreground sm:min-h-10 sm:text-sm [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
      ...
      <SelectPrimitive.ItemIndicator
        render={
          <span className="pointer-events-none absolute right-2.5 top-1/2 flex size-4 -translate-y-1/2 items-center justify-center text-accent" />
        }
      >
