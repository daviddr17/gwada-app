...
On-Call ist E-Mail an Superadmins. Ein unverändertes 24h-SLO (gleiche Zahl später Bestätigungen, kein Cron-Lag, nichts hängend) wird höchstens einmal am Tag wiederholt. Hängender Versand oder Lag der Zustell-Crons eskaliert weiter alle 10 Minuten mit Betreff `ESKALATION` ab der 2. Mail. GitHub-Sync-Lag (Inbox/News/Reviews/…) löst keine Mail aus. Ein Slack-/Telegram-Webhook kommt als eigene Platform-Integration — nicht über `.env`.
