import { reservationActiveAtInstant } from "./local-day-bounds";
...
export type TableOccupancyReservationWithStatus = TableOccupancyReservation & {
  status?: { code: string } | null;
  reservation_statuses?: { code: string } | null;
};

export type DiningTableLike = {
  id: string;
  capacity?: number;
};
...
function readStatusCode(
  r: TableOccupancyReservationWithStatus,
): string | null {
...
export function nextReservationAtTable<
  T extends TableOccupancyReservationWithStatus,
>(
  reservations: T[],
  tableId: string,
  ...
  for (const r of reservations) {
    if (r.dining_table_id !== tableId) continue;
    const code = readStatusCode(r);
    if (!reservationOccupiesTableAtInstant(code, options)) continue;
    const startMs = new Date(r.starts_at).getTime();
    if (startMs <= nowMs) continue;
    if (
      !best ||
      startMs < new Date(best.starts_at).getTime()
    ) {
      best = r;
    }
  }
