...
const catRows = [
  ["starters", "Vorspeisen", 0, "food"],
  ["mains", "Hauptgerichte", 1, "food"],
  ["sides", "Beilagen", 2, "food"],
  ["desserts", "Desserts", 3, "food"],
  ["drinks", "Getränke", 4, "beverage"],
];
...
sql += mainCatRows
  .map(
    ([key, name, ord]) =>
      `    ('${MAIN[key]}', rid, '${esc(name)}', ${ord}, true)`,
  )
  .join(",\n");
...
sql += catRows
  .map(
    ([key, name, ord, mainKey]) =>
      `    ('${CAT[key]}', rid, '${esc(name)}', ${ord}, true, '${MAIN[mainKey]}')`,
  )
  .join(",\n");
