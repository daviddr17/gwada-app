...
https_rule_pattern = (
    rf"            - 'traefik\.http\.routers\.https-0-{re.escape(app_id)}\.rule=Host\(`[^`]+`\) && PathPrefix\(`/`\)'"
)
https_rule_replacement = (
    f"            - 'traefik.http.routers.https-0-{app_id}.rule=Host(`{fqdn}`) && PathPrefix(`/`)'"
)
http_rule = (
...
text, n_https = re.subn(https_rule_pattern, https_rule_replacement, text, count=1)
...
if n_https:
    print(f"Traefik-HTTPS-Host auf {fqdn} gesetzt.")
else:
