-- Display-Modul: Bestand & Bestellung (Touch-Inventur / Bestellmengen)

do $$ begin
  alter type public.display_module add value if not exists 'inventory';
exception
  when duplicate_object then null;
end $$;

insert into public.restaurant_position_permissions (position_id, permission_key)
select rp.id, 'display.inventory'
from public.restaurant_positions rp
cross join (values ('display.inventory')) as perm(key)
