-- Repair Live: Version 20260611120000 war dort restaurant_menu_settings, lokal accounting_module.
-- Wendet Buchführungs-Schema idempotent an, falls Tabellen noch fehlen.
...
create index if not exists accounting_tax_rates_restaurant_id_idx
  on public.accounting_tax_rates (restaurant_id);

drop trigger if exists accounting_tax_rates_set_updated_at on public.accounting_tax_rates;
...
drop policy if exists accounting_tax_rates_staff_select on public.accounting_tax_rates;
create policy accounting_tax_rates_staff_select
  on public.accounting_tax_rates for select
  to authenticated
  using (public.auth_is_restaurant_staff(restaurant_id));

drop policy if exists accounting_tax_rates_staff_write on public.accounting_tax_rates;
...
create table if not exists public.accounting_units (
  id uuid primary key default gen_random_uuid(),
  restaurant_id uuid not null references public.restaurants (id) on delete cascade,
  name text not null,
  sort_order integer not null default 0,
...
create unique index if not exists accounting_units_restaurant_name_active_uidx
  on public.accounting_units (restaurant_id, lower(name))
  where not archived;

create index if not exists accounting_units_restaurant_id_idx
  on public.accounting_units (restaurant_id);

drop trigger if exists accounting_units_set_updated_at on public.accounting_units;
...
drop policy if exists accounting_units_staff_select on public.accounting_units;
create policy accounting_units_staff_select
  on public.accounting_units for select
  to authenticated
  using (public.auth_is_restaurant_staff(restaurant_id));

drop policy if exists accounting_units_staff_write on public.accounting_units;
...
create table if not exists public.accounting_articles (
  id uuid primary key default gen_random_uuid(),
  restaurant_id uuid not null references public.restaurants (id) on delete cascade,
  name text not null,
  description text,
...
create index if not exists accounting_articles_restaurant_id_idx
  on public.accounting_articles (restaurant_id);

drop trigger if exists accounting_articles_set_updated_at on public.accounting_articles;
...
drop policy if exists accounting_articles_staff_select on public.accounting_articles;
create policy accounting_articles_staff_select
  on public.accounting_articles for select
  to authenticated
  using (public.auth_is_restaurant_staff(restaurant_id));

drop policy if exists accounting_articles_staff_write on public.accounting_articles;
...
create unique index if not exists accounting_invoices_restaurant_external_uidx
  on public.accounting_invoices (restaurant_id, source, external_id)
  where external_id is not null;

create index if not exists accounting_invoices_restaurant_id_idx
...
drop policy if exists accounting_invoices_staff_select on public.accounting_invoices;
create policy accounting_invoices_staff_select
  on public.accounting_invoices for select
  to authenticated
  using (public.auth_is_restaurant_staff(restaurant_id));

drop policy if exists accounting_invoices_staff_write on public.accounting_invoices;
...
create table if not exists public.accounting_quotations (
  id uuid primary key default gen_random_uuid(),
  restaurant_id uuid not null references public.restaurants (id) on delete cascade,
  source text not null default 'gwada'
    check (source in ('gwada', 'lexoffice')),
...
create unique index if not exists accounting_quotations_restaurant_external_uidx
  on public.accounting_quotations (restaurant_id, source, external_id)
  where external_id is not null;

create index if not exists accounting_quotations_restaurant_id_idx
  on public.accounting_quotations (restaurant_id, voucher_date desc);

drop trigger if exists accounting_quotations_set_updated_at on public.accounting_quotations;
...
drop policy if exists accounting_quotations_staff_select on public.accounting_quotations;
create policy accounting_quotations_staff_select
  on public.accounting_quotations for select
  to authenticated
  using (public.auth_is_restaurant_staff(restaurant_id));

drop policy if exists accounting_quotations_staff_write on public.accounting_quotations;
...
create unique index if not exists accounting_vouchers_restaurant_external_uidx
  on public.accounting_vouchers (restaurant_id, source, external_id)
  where external_id is not null;

create index if not exists accounting_vouchers_restaurant_id_idx
  on public.accounting_vouchers (restaurant_id, voucher_date desc);

drop trigger if exists accounting_vouchers_set_updated_at on public.accounting_vouchers;
...
drop policy if exists accounting_vouchers_staff_select on public.accounting_vouchers;
create policy accounting_vouchers_staff_select
  on public.accounting_vouchers for select
  to authenticated
  using (public.auth_is_restaurant_staff(restaurant_id));

drop policy if exists accounting_vouchers_staff_write on public.accounting_vouchers;
...
drop policy if exists accounting_vouchers_storage_select_staff on storage.objects;
...
drop policy if exists accounting_vouchers_storage_insert_staff on storage.objects;
...
drop policy if exists accounting_vouchers_storage_delete_staff on storage.objects;
