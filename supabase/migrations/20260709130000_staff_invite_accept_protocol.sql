-- Protokoll: Einladung angenommen (robust gegen Regressionen in accept_staff_invite)
...
create or replace function public.log_staff_invite_accepted(
  p_restaurant_id uuid,
  p_staff_id uuid,
  p_actor_user_id uuid,
  p_restaurant_position_id uuid
)
returns void
language plpgsql
security definer
...
begin
  ...
  where rp.id = p_restaurant_position_id;
...
revoke all on function public.log_staff_invite_accepted(uuid, uuid, uuid, uuid) from public;
grant execute on function public.log_staff_invite_accepted(uuid, uuid, uuid, uuid) to authenticated, service_role;
