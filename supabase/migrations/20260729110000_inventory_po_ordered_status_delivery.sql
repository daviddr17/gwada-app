-- Purchase orders: Status „ordered“ + Liefer-Antwort pro Position
...
alter table public.inventory_purchase_order_lines
  drop constraint if exists inventory_pol_delivery_status_check;

alter table public.inventory_purchase_order_lines
  add constraint inventory_pol_delivery_status_check
  check (
    delivery_status is null
    or delivery_status in ('delivered', 'not_delivered', 'partial')
  );

alter table public.inventory_purchase_order_lines
  add column if not exists delivered_quantity numeric(14, 4);
